  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!isNaN(num) && num >= 1 && num <= 100) {
      ...
      if (next) {
        if (next.match(/^[A-D]$/)) {
          gabarito[num] = next;
        } else if (next === 'Anulada' || next === '- Anulada' || next === '-') {
          gabarito[num] = null;
        }
      }
    }
...
function parseGabaritoVerticalDash(text) {
  // Format for 2023 gabaritos: 
  // First block: "Questão" header, then numbers 1-20
  // Then "Gabarito" header, then answers for 1-20
  // Then repeats for 21-40, 41-60, 61-80, 81-100
  const gabarito = {};
  const lines = text.split('\n').map(l => l.trim()).filter(l => l);
  
  // Collect all question numbers and all answers separately
  const allNumbers = [];
  const allAnswers = [];
  let inNumbers = false;
  let inAnswers = false;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line === 'Questão' || line.startsWith('Questão')) {
      inNumbers = true;
      inAnswers = false;
      continue;
    }
    if (line === 'Gabarito' || line.startsWith('Gabarito')) {
      inNumbers = false;
      inAnswers = true;
      continue;
    }
    if (line.startsWith('PROVA') || line.startsWith('EDIÇÃO') || line.startsWith('GABARITO DEF')) {
      continue;
    }
    ...
    if (inAnswers) {
      ...
      } else if (line === '-' || line.includes('Anulada')) {
        allAnswers.push(null);
      }
    }
  }
  
  // Map numbers to answers
  for (let i = 0; i < allNumbers.length && i < allAnswers.length; i++) {
    gabarito[allNumbers[i]] = allAnswers[i];
  }
...
function parseQuestions(text, source, gabaritoMap, yearNum) {
  const questions = [];
  ...
  for (let i = 1; i < parts.length; i += 2) {
    ...
    for (const line of lines) {
      ...
      if (trimmed.match(/^\d{4}\s*$/)) continue;
...
function classifyArea(text) {
  ...
  if (lower.includes('gestante') || lower.includes('pré-natal') || lower.includes('parto') || lower.includes('puérpera') || lower.includes('obstetrícia') || lower.includes('gravidez') || lower.includes('abortamento') || lower.includes('eclâmpsia') || lower.includes('amniorrexe') || lower.includes('ginecolog')) return 'Ginecologia e Obstetrícia';
  if (lower.includes('criança') || lower.includes('lactente') || lower.includes('recém-nascido') || lower.includes('neonato') || lower.includes('pediatr') || lower.includes('adolescente') || lower.includes('meses de vida') || lower.includes('meses de idade') || (lower.includes('anos de idade') && (lower.includes('menino') || lower.includes('menina')))) return 'Pediatria';
  if (lower.includes('cirurgi') || lower.includes('laparotomia') || lower.includes('herniorrafia') || lower.includes('colecistectomia') || lower.includes('apendicectomia') || lower.includes('fratura') || lower.includes('osteossíntese') || lower.includes('pós-operatório')) return 'Cirurgia';
  if (lower.includes('unidade básica') || lower.includes('ubs') || lower.includes('atenção primária') || lower.includes('saúde da família') || lower.includes('epidemiolog') || lower.includes('vigilância') || lower.includes('sus') || lower.includes('programa nacional') || lower.includes('cobertura vacinal') || lower.includes('estratégia saúde') || lower.includes('saúde coletiva') || lower.includes('prevenção') || lower.includes('rastreamento')) return 'Saúde Coletiva';
...
const enamed_questions = parseQuestions(enamed_text, 'ENAMED_2025', enamed_gabarito, 2025);
...
try {
  ...
  console.log('Gabarito REVALIDA 2024/2 não encontrado');
}
const revalida_2024_2_questions = parseQuestions(revalida_2024_2_text, 'REVALIDA_2024_2', revalida_2024_2_gabarito, 2024);
...
const revalida_2024_1_questions = parseQuestions(revalida_2024_1_text, 'REVALIDA_2024_1', revalida_2024_1_gabarito, 2024);
console.log(`REVALIDA 2024/1: ${revalida_2024_1_questions.length} questões parseadas`);

// 4. REVALIDA 2023/2
const revalida_2023_2_text = readFileSync('/home/ubuntu/medfocus-app/research/revalida_2023_2_prova.txt', 'utf-8');
const revalida_2023_2_gabarito_text = readFileSync('/home/ubuntu/medfocus-app/research/revalida_2023_2_gabarito.txt', 'utf-8');
const revalida_2023_2_gabarito = parseGabaritoVerticalDash(revalida_2023_2_gabarito_text);
console.log(`Gabarito REVALIDA 2023/2: ${Object.keys(revalida_2023_2_gabarito).length} questões (${Object.values(revalida_2023_2_gabarito).filter(v => v === null).length} anuladas)`);
const revalida_2023_2_questions = parseQuestions(revalida_2023_2_text, 'REVALIDA_2023_2', revalida_2023_2_gabarito, 2023);
console.log(`REVALIDA 2023/2: ${revalida_2023_2_questions.length} questões parseadas`);

// 5. REVALIDA 2023/1
const revalida_2023_1_text = readFileSync('/home/ubuntu/medfocus-app/research/revalida_2023_1_prova.txt', 'utf-8');
const revalida_2023_1_gabarito_text = readFileSync('/home/ubuntu/medfocus-app/research/revalida_2023_1_gabarito.txt', 'utf-8');
const revalida_2023_1_gabarito = parseGabaritoVerticalDash(revalida_2023_1_gabarito_text);
console.log(`Gabarito REVALIDA 2023/1: ${Object.keys(revalida_2023_1_gabarito).length} questões (${Object.values(revalida_2023_1_gabarito).filter(v => v === null).length} anuladas)`);
const revalida_2023_1_questions = parseQuestions(revalida_2023_1_text, 'REVALIDA_2023_1', revalida_2023_1_gabarito, 2023);
console.log(`REVALIDA 2023/1: ${revalida_2023_1_questions.length} questões parseadas`);

// ========== COMBINE ALL ==========

const allQuestions = [
  ...enamed_questions, 
  ...revalida_2024_2_questions, 
  ...revalida_2024_1_questions,
  ...revalida_2023_2_questions,
  ...revalida_2023_1_questions
];
